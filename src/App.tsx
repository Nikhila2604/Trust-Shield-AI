...
export default function App() {
  // Page selector state: 'home' | 'analyze' | 'chat'
  const [currentPage, setCurrentPage] = useState<'home' | 'analyze' | 'chat'>('home');
  ...
  const loadScanHistory = async () => {
    try {
      ...
      if (res.ok) {
        ...
        // We REMOVED loading the default item on startup! 
        // Thus, the results card will only display after the user actually performs an action.
      }
    } catch (err) {
      ...
      <div>
        ...
        <nav className="sticky top-0 z-50 bg-white/90 backdrop-blur-md border-b border-slate-200 px-4 sm:px-6 lg:px-8 py-3.5 shadow-xs">
          <div className="max-w-7xl mx-auto flex items-center justify-between">
            ...
            <div className="flex items-center gap-2 md:gap-4 bg-slate-100/80 p-1 rounded-2xl border border-slate-200">
              ...
              <button
                ...
                className={`flex items-center gap-1.5 px-3 py-1.5 sm:px-4 sm:py-2 rounded-xl text-xs font-extrabold transition-all duration-300 cursor-pointer ${
                  currentPage === 'home'
